...
def check_victory(game):
    ...
    # after poping out one block, both player satisfy the winning condition,
    # the player execute the move wins
    if 2 in winner and 1 in winner:
        return 3-game.turn
    elif 2 in winner and not(1 in winner):
        return 2
    elif not (2 in winner) and 1 in winner:
        return 1
...
def human_move(game):
    ...
    col = choose_col(game)
    pop = bool(choose_add_on_method(game))
...
def menu():
    ...
    while not (game.rows.isdigit() == True and int(game.rows) >= 3):
        ...
        print('Your input value is invalid! Please enter again')
        # User will be prompted to key in the valid values if input error occurs.
        game.rows = input()
    else:
...
def computer_level_easy(game):
    ...
    cpu_level_easy_pop = bool(randint(0, 1))
            ...
            if check_move(game, col, pop):
                ...
                if check_victory(simulate_game) == opponent_turn:
                    ...
                    # the move includes poping out a bottom block
                        ...
                        simulate_game_pop.turn=game.turn
                        # go through all possibe moves to find the defensive move
                        for i in range(simulate_game_pop.cols):
                            if i==col: #the column can not be the previous column itself
                                continue
                            elif check_move(simulate_game_pop, i, True):
                                simulate_game_pop = simulate_apply_move(simulate_game_pop, i, True)
...
def computer_level_difficult(game):
            ...
            if check_move(game, col, pop):
                ...
                if check_victory(simulate_game) == opponent_turn:
                    if not pop:
                        ...
                        game.turn = update_game_turn(game)
                        flag = 1
                        simulate_game.turn = game.turn
                        simulate_game_pop = simulation(simulate_game)
                        for i in range(simulate_game_pop.cols):
                            if i==col:
                                continue
                            elif check_move(simulate_game_pop, i, 1):
                                simulate_game_pop = simulate_apply_move(simulate_game_pop, i, 1)
                                if check_victory(simulate_game_pop) != opponent_turn:
                                    flag = 0
                                    return (i, bool(1))
                        if flag == 1:
    ...
    for pop in range(0, 2):
        for col in range(game.cols):
            col_score=100-(col-(game.cols//2))**2 #heuristically the central blocks will have more scores than edge scores
            # the lower rows will have higher scores
            i=0
            while game.mat[i][col]!=0:
                i+=1
            row=i
            row_score=4-i
            if check_move(game, col, pop):
